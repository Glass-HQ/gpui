...
use crate::native_controls_common::{
    define_cleanup_view_only, define_cleanup_with_target, has_combo_box_callbacks,
    has_text_field_callbacks, optional_target, replace_target_if_present,
};
...
define_cleanup_view_only!(
    ...
    id,
    remove_from_parent,
    release_native_progress_indicator
);
define_cleanup_view_only!(
    cleanup_image_view,
    native_controls,
...
define_cleanup_view_only!(
    ...
    id,
    remove_from_parent,
    release_native_glass_effect_view
);
define_cleanup_view_only!(
    cleanup_stack_view,
    native_controls,
...
) -> super::native_controls::TextFieldCallbacks {
    super::native_controls::TextFieldCallbacks {
...
) -> super::native_controls::ComboBoxCallbacks {
    super::native_controls::ComboBoxCallbacks {
...
) -> super::native_controls::TrackingViewCallbacks {
    super::native_controls::TrackingViewCallbacks {
...
) -> Vec<super::native_controls::NativeOutlineNodeData> {
    nodes
        .iter()
        .map(|node| super::native_controls::NativeOutlineNodeData {
...
) -> super::native_controls::NativeCollectionItemStyleData {
    match style {
        CollectionItemStyle::Label => super::native_controls::NativeCollectionItemStyleData::Label,
        CollectionItemStyle::Card => super::native_controls::NativeCollectionItemStyleData::Card,
    }
}

fn convert_menu_items(
    items: &[NativeMenuItemData],
) -> Vec<super::native_controls::NativeMenuItemData> {
    items
        .iter()
        .map(|item| match item {
            NativeMenuItemData::Action {
                ...
                enabled,
                icon,
            } => super::native_controls::NativeMenuItemData::Action {
                title: title.clone(),
                enabled: *enabled,
                icon: icon.clone(),
            },
            NativeMenuItemData::Submenu {
                ...
                icon,
                items,
            } => super::native_controls::NativeMenuItemData::Submenu {
                title: title.clone(),
                enabled: *enabled,
                icon: icon.clone(),
                items: convert_menu_items(items),
            },
            NativeMenuItemData::Separator => super::native_controls::NativeMenuItemData::Separator,
        })
        .collect()
}

fn convert_alert_style(style: AlertStyle) -> super::native_controls::NativeAlertStyleRaw {
    match style {
        AlertStyle::Warning => super::native_controls::NativeAlertStyleRaw::Warning,
        AlertStyle::Informational => super::native_controls::NativeAlertStyleRaw::Informational,
        AlertStyle::Critical => super::native_controls::NativeAlertStyleRaw::Critical,
    }
}

fn convert_panel_style(style: PanelStyle) -> super::native_controls::NativePanelStyle {
    match style {
        PanelStyle::Titled => super::native_controls::NativePanelStyle::Titled,
        PanelStyle::Borderless => super::native_controls::NativePanelStyle::Borderless,
        PanelStyle::Hud => super::native_controls::NativePanelStyle::Hud,
        PanelStyle::Utility => super::native_controls::NativePanelStyle::Utility,
    }
}

fn convert_panel_level(level: PanelLevel) -> super::native_controls::NativePanelLevel {
    match level {
        PanelLevel::Normal => super::native_controls::NativePanelLevel::Normal,
        PanelLevel::Floating => super::native_controls::NativePanelLevel::Floating,
        PanelLevel::ModalPanel => super::native_controls::NativePanelLevel::ModalPanel,
        PanelLevel::PopUpMenu => super::native_controls::NativePanelLevel::PopUpMenu,
        PanelLevel::Custom(value) => super::native_controls::NativePanelLevel::Custom(value),
    }
}

fn convert_panel_material(
    material: Option<PanelMaterial>,
) -> Option<super::native_controls::NativePanelMaterial> {
    material.map(|material| match material {
        PanelMaterial::HudWindow => super::native_controls::NativePanelMaterial::HudWindow,
        PanelMaterial::Popover => super::native_controls::NativePanelMaterial::Popover,
        PanelMaterial::Sidebar => super::native_controls::NativePanelMaterial::Sidebar,
        PanelMaterial::UnderWindow => super::native_controls::NativePanelMaterial::UnderWindow,
    })
}
    ...
    fn update_button(
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    state,
                    config.on_click,
                    native_controls::release_native_button_target,
                    |callback| native_controls::set_native_button_action(view, callback),
                );
                ...
                let target = optional_target(config.on_click, |callback| {
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_checkbox_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {
                ...
                native_controls::set_native_checkbox_state(view, config.checked);
                native_controls::set_native_control_enabled(view, config.enabled);
                let target = optional_target(config.on_change, |callback| {
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_switch_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_slider_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {
                let view =
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_stepper_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_segmented_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {
                ...
                native_controls::set_native_control_enabled(view, config.enabled);
                let target = optional_target(config.on_select, |callback| {
        ...
        unsafe {
            if state.is_initialized() {
                ...
                replace_target_if_present(
                    ...
                    |callback| native_controls::set_native_popup_action(view, callback),
                );
                ensure_attached(state, parent, bounds, scale);
            } else {

...
unsafe fn build_outline_view_class() {
    unsafe {
        let mut decl = ClassDecl::new("GPUINativeOutlineView", class!(NSOutlineView)).unwrap();
...
pub unsafe fn create_native_outline_view() -> id {
    unsafe {
        use super::super::ns_string;
...
pub unsafe fn set_native_outline_items(
    scroll_view: id,
...
pub unsafe fn sync_native_outline_column_width(scroll_view: id) {
    unsafe {
        let outline = outline_from_scroll(scroll_view);
        if outline == nil {
            return;
...
pub unsafe fn set_native_outline_highlight_style(scroll_view: id, style: i64) {
    unsafe {
        let outline = outline_from_scroll(scroll_view);
...
pub unsafe fn set_native_outline_row_height(scroll_view: id, row_height: f64) {
    unsafe {
        let outline = outline_from_scroll(scroll_view);
...
pub unsafe fn release_native_outline_target(target: *mut c_void) {
    unsafe {
        if target.is_null() {
            return;
        }
...
pub unsafe fn release_native_outline_view(scroll_view: id) {
    unsafe {
        let outline = outline_from_scroll(scroll_view);

...
pub unsafe fn create_native_tracking_view() -> id {
    unsafe {
        ...
        let area: id = msg_send![class!(NSTrackingArea), alloc];
        let area: id =
            msg_send![area, initWithRect: bounds options: options owner: view userInfo: nil];
...
pub unsafe fn set_native_tracking_view_callbacks(
    view: id,
...
pub unsafe fn release_native_tracking_view_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
...
pub unsafe fn release_native_tracking_view(view: id) {
    unsafe {
        if view != nil {

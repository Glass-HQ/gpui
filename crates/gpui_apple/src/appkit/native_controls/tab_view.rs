...
pub unsafe fn create_native_tab_view() -> id {
    unsafe {
...
pub unsafe fn set_native_tab_view_items(tab_view: id, labels: &[&str], selected_index: usize) {
    unsafe {
...
pub unsafe fn set_native_tab_view_selected(tab_view: id, index: usize) {
    unsafe {
        let count: i64 = msg_send![tab_view, numberOfTabViewItems];
        if count <= 0 {
            return;
...
pub unsafe fn set_native_tab_view_action(
    tab_view: id,
    on_select: Option<Box<dyn Fn(usize)>>,
) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_tab_view_target(target: *mut c_void) {
    unsafe {
        if target.is_null() {
            return;
        }
...
pub unsafe fn release_native_tab_view(tab_view: id) {
    unsafe {

...
pub unsafe fn create_native_stepper(min: f64, max: f64, value: f64, increment: f64) -> id {
    unsafe {
...
pub unsafe fn set_native_stepper_min(stepper: id, min: f64) {
    unsafe {
        let _: () = msg_send![stepper, setMinValue: min];
    }
}

pub unsafe fn set_native_stepper_max(stepper: id, max: f64) {
    unsafe {
        let _: () = msg_send![stepper, setMaxValue: max];
    }
}

pub unsafe fn set_native_stepper_value(stepper: id, value: f64) {
    unsafe {
        let _: () = msg_send![stepper, setDoubleValue: value];
    }
}

pub unsafe fn set_native_stepper_increment(stepper: id, increment: f64) {
    unsafe {
        let _: () = msg_send![stepper, setIncrement: increment];
    }
}

pub unsafe fn set_native_stepper_wraps(stepper: id, wraps: bool) {
    unsafe {
        let _: () = msg_send![stepper, setValueWraps: wraps as i8];
    }
}

pub unsafe fn set_native_stepper_autorepeat(stepper: id, autorepeat: bool) {
    unsafe {
        let _: () = msg_send![stepper, setAutorepeat: autorepeat as i8];
    }
}

pub unsafe fn set_native_stepper_action(stepper: id, callback: Box<dyn Fn(f64)>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_stepper_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
...
pub unsafe fn release_native_stepper(stepper: id) {
    unsafe {

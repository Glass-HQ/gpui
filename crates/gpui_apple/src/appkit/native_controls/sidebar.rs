...
pub unsafe fn configure_sidebar_window(
    host_view: id,
    parent_view: id,
    embed_in_sidebar: bool,
...
pub unsafe fn set_sidebar_width(host_view: id, sidebar_width: f64, min_width: f64, max_width: f64) {
    unsafe {
        let Some(host_data) = host_data_mut(host_view) else {
            return;
        };
...
pub unsafe fn set_sidebar_collapsed(
    host_view: id,
    collapsed: bool,
    expanded_width: f64,
    min_width: f64,
...
pub unsafe fn set_inspector_collapsed(
    host_view: id,
    collapsed: bool,
    expanded_width: f64,
    min_width: f64,
...
pub unsafe fn set_sidebar_items(
    host_view: id,
    items: &[&str],
...
pub unsafe fn sidebar_requires_rebuild(
    host_view: id,
    sidebar_on_trailing: bool,
    embed_in_sidebar: bool,
    has_inspector: bool,
...
pub unsafe fn update_sidebar_header_callback(host_view: id, callback: Option<Box<dyn Fn(usize)>>) {
    unsafe {
        let Some(host_data) = host_data_mut(host_view) else {
            return;
        };
...
pub unsafe fn set_sidebar_header(host_view: id, title: Option<&str>, button_symbols: &[&str]) {
    unsafe {
        use super::super::ns_string;
...
pub unsafe fn set_sidebar_header_height(host_view: id, height: f64) {
    unsafe {
        let Some(host_data) = host_data_mut(host_view) else {
            return;
        };
...
pub unsafe fn embed_sidebar_surface_view(host_view: id, surface_view: id) {
    unsafe {
...
pub unsafe fn embed_sidebar_header_surface_view(host_view: id, surface_view: id) {
    unsafe {
        if host_view == nil || surface_view == nil {
            return;
        }
...
pub unsafe fn embed_inspector_surface_view(host_view: id, surface_view: id) {
    unsafe {
        if host_view == nil || surface_view == nil {
            return;
        }
...
pub unsafe fn set_sidebar_background_color(host_view: id, r: f64, g: f64, b: f64, a: f64) {
    unsafe {
        let Some(host_data) = host_data_mut(host_view) else {
            return;
        };
...
pub unsafe fn clear_sidebar_background_color(host_view: id) {
    unsafe {
        let Some(host_data) = host_data_mut(host_view) else {
            return;
        };
...
pub unsafe fn release_sidebar_target(target: *mut c_void) {
    unsafe {
        if target.is_null() {
            return;
        }
...
pub unsafe fn release_sidebar_view(host_view: id) {
    unsafe {
        if host_view == nil {
            return;
        }

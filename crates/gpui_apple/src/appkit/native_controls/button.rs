...
static mut BUTTON_CLASS: *const Class = ptr::null();
...
unsafe fn build_button_target_class() {
    unsafe {
        ...
        let mut button_decl = ClassDecl::new("GPUINativeButton", class!(NSButton)).unwrap();
        button_decl.add_method(
            sel!(scrollWheel:),
            button_scroll_wheel as extern "C" fn(&Object, Sel, id),
        );
        BUTTON_CLASS = button_decl.register();
    }
}
...
extern "C" fn button_scroll_wheel(this: &Object, _sel: Sel, event: id) {
    unsafe {
        let superview: id = msg_send![this, superview];
        if superview != nil {
            let _: () = msg_send![superview, scrollWheel: event];
        } else {
            let _: () = msg_send![super(this, class!(NSButton)), scrollWheel: event];
        }
    }
}
...
pub unsafe fn create_native_button(title: &str) -> id {
    unsafe {
...
pub unsafe fn set_native_button_title(button: id, title: &str) {
    unsafe {
        use super::super::ns_string;
        let _: () = msg_send![button, setTitle: ns_string(title)];
    }
...
pub unsafe fn set_native_button_action(button: id, callback: Box<dyn Fn()>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_button_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
...
pub unsafe fn release_native_button(button: id) {
    unsafe {
...
pub unsafe fn set_native_button_bezel_style(button: id, bezel_style: i64) {
    unsafe {
...
pub unsafe fn set_native_button_bordered(button: id, bordered: bool) {
    unsafe {
...
pub unsafe fn set_native_button_bezel_color(button: id, r: f64, g: f64, b: f64, a: f64) {
    unsafe {
...
pub unsafe fn set_native_button_shows_border_on_hover(button: id, shows: bool) {
    unsafe {
...
pub unsafe fn set_native_button_bezel_color_accent(button: id) {
    unsafe {
...
pub unsafe fn set_native_button_content_tint_color(button: id, r: f64, g: f64, b: f64, a: f64) {
    unsafe {
        let color: id = msg_send![class!(NSColor), colorWithSRGBRed:r green:g blue:b alpha:a];
...
pub unsafe fn set_native_button_sf_symbol(button: id, symbol_name: &str, image_only: bool) {
    unsafe {
        use super::super::ns_string;

...
pub enum NativeCollectionItemStyleData {
    Label,
    Card,
}
...
pub unsafe fn create_native_collection_view() -> id {
    unsafe {
...
pub unsafe fn set_native_collection_layout(
    scroll_view: id,
    width: f64,
    columns: usize,
    item_height: f64,
...
pub unsafe fn set_native_collection_data_source(
    scroll_view: id,
    items: &[&str],
    selected: Option<usize>,
    item_style: NativeCollectionItemStyleData,
...
pub unsafe fn release_native_collection_target(target: *mut c_void) {
    unsafe {
        if target.is_null() {
            return;
        }
...
pub unsafe fn release_native_collection_view(scroll_view: id) {
    unsafe {
        let collection = collection_from_scroll(scroll_view);

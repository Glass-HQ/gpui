...
pub unsafe fn create_native_alert(
...
pub unsafe fn run_native_alert_modal(alert: id) -> i64 {
    unsafe {
...
pub unsafe fn run_native_alert_as_sheet(
    alert: id,
    parent_window: id,
...
/// Releases an alert.
pub unsafe fn release_native_alert(alert: id) {
    unsafe {
        if alert != nil {
            let _: () = msg_send![alert, release];
        }

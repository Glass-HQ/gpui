...
pub unsafe fn create_native_table_view() -> id {
    unsafe {
        use super::super::ns_string;
...
pub unsafe fn set_native_table_column_title(scroll_view: id, title: &str) {
    unsafe {
        use super::super::ns_string;
...
pub unsafe fn set_native_table_column_width(scroll_view: id, width: f64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
        let column = primary_table_column(table);
        if column != nil {
...
pub unsafe fn set_native_table_items(
    scroll_view: id,
    items: &[&str],
...
pub unsafe fn set_native_table_row_height(scroll_view: id, row_height: f64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_row_size_style(scroll_view: id, row_size_style: i64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_style(scroll_view: id, style: i64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_selection_highlight_style(scroll_view: id, highlight_style: i64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_grid_style(scroll_view: id, grid_style_mask: u64) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_uses_alternating_rows(scroll_view: id, uses: bool) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_allows_multiple_selection(scroll_view: id, allows_multiple: bool) {
    unsafe {
        let table = table_from_scroll(scroll_view);
...
pub unsafe fn set_native_table_show_header(scroll_view: id, show_header: bool) {
    unsafe {
        let table = table_from_scroll(scroll_view);
        if show_header {
...
pub unsafe fn release_native_table_target(target: *mut c_void) {
    unsafe {
        if target.is_null() {
            return;
        }
...
pub unsafe fn release_native_table_view(scroll_view: id) {
    unsafe {
        let table = table_from_scroll(scroll_view);

...
pub unsafe fn create_native_combo_box(
    items: &[&str],
...
pub unsafe fn set_native_combo_box_items(_combo: id, _items: &[&str]) {
    // No-op — items would be set on a UIPickerView inputView
}

/// Sets the selected index by updating the text field value.
pub unsafe fn set_native_combo_box_selected(_combo: id, _index: usize) {
...
/// No-op on iOS.
pub unsafe fn set_native_combo_box_completes(_combo: id, _completes: bool) {}

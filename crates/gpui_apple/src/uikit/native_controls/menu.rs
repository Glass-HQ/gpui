...
pub unsafe fn create_native_menu_button(title: &str) -> id {
    unsafe {
...
pub unsafe fn create_native_context_menu_button(title: &str) -> id {
    unsafe { create_native_menu_button(title) }
}

/// Sets the menu button title.
pub unsafe fn set_native_menu_button_title(button: id, title: &str) {
    unsafe {
        let _: () = msg_send![button, setTitle: ns_string(title) forState: 0u64];
    }
}
...
pub unsafe fn set_native_menu_button_items(button: id, items: &[&str], target: *mut c_void) {
    unsafe {
...
pub unsafe fn create_native_menu_target(callback: Box<dyn Fn(usize)>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_menu_button_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
            let target = target as id;
            let callback_ptr: *mut c_void = *(*target).get_ivar(CALLBACK_IVAR);
...
pub unsafe fn release_native_menu_button(button: id) {
    unsafe {
        if !button.is_null() {

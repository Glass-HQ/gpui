...
pub unsafe fn create_native_checkbox(title: &str) -> id {
    unsafe {
...
pub unsafe fn set_native_checkbox_title(checkbox: id, title: &str) {
    unsafe {
...
pub unsafe fn set_native_checkbox_state(checkbox: id, checked: bool) {
    unsafe {
...
pub unsafe fn set_native_checkbox_action(checkbox: id, callback: Box<dyn Fn(bool)>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_checkbox_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
...
pub unsafe fn release_native_checkbox(checkbox: id) {
    unsafe {
        if !checkbox.is_null() {
            let _: () = msg_send![checkbox, release];
        }

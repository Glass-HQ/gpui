...
pub unsafe fn create_native_slider(min: f64, max: f64, value: f64) -> id {
    unsafe {
...
pub unsafe fn set_native_slider_value(slider: id, value: f64) {
    unsafe {
        let _: () = msg_send![slider, setValue: value as f32 animated: false as i8];
...
pub unsafe fn set_native_slider_min(slider: id, min: f64) {
    unsafe {
        let _: () = msg_send![slider, setMinimumValue: min as f32];
    }
}
...
pub unsafe fn set_native_slider_max(slider: id, max: f64) {
    unsafe {
        let _: () = msg_send![slider, setMaximumValue: max as f32];
    }
}
...
pub unsafe fn set_native_slider_continuous(slider: id, continuous: bool) {
    unsafe {
...
pub unsafe fn set_native_slider_tick_marks(_slider: id, _count: i64, _snap: bool) {}

/// Sets the slider's target/action callback.
pub unsafe fn set_native_slider_action(slider: id, callback: Box<dyn Fn(f64)>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_slider_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
...
/// Releases a UISlider.
pub unsafe fn release_native_slider(slider: id) {
    unsafe {
        if !slider.is_null() {

...
pub unsafe fn create_native_popup_button(items: &[&str], selected_index: usize) -> id {
    unsafe {
...
pub unsafe fn set_native_popup_items(popup: id, items: &[&str], target: *mut c_void) {
    unsafe {
...
pub unsafe fn set_native_popup_selected(popup: id, index: usize) {
    unsafe {
...
pub unsafe fn set_native_popup_action(callback: Box<dyn Fn(usize)>) -> *mut c_void {
    unsafe {
...
pub unsafe fn release_native_popup_target(target: *mut c_void) {
    unsafe {
        if !target.is_null() {
            let target = target as id;
            let callback_ptr: *mut c_void = *(*target).get_ivar(CALLBACK_IVAR);
...
pub unsafe fn release_native_popup_button(popup: id) {
    unsafe {
        if !popup.is_null() {

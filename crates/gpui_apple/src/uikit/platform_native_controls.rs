...
use crate::native_controls_common::{
    ...
    has_text_field_callbacks, normalized_progress, optional_target, replace_target_if_present,
    reset_target,
};

use super::native_controls::{self, id};
...
define_cleanup_with_target!(
    ...
    native_controls,
    id,
    remove_native_view_from_parent,
    release_native_button_target,
...
define_cleanup_with_target!(
    ...
    id,
    remove_native_view_from_parent,
    release_native_sidebar_target,
    release_native_sidebar_view
);
        ...
        unsafe {
            if state.is_initialized() {
                ...
                reset_target(
                    ...
                    |callback| native_controls::set_native_popup_action(callback),
                );
                ...
                let target = optional_target(config.on_select, |callback| {
                    native_controls::set_native_popup_action(callback)
                });
                native_controls::set_native_popup_items(view, config.items, target);
                native_controls::set_native_popup_selected(view, config.selected_index);
                native_controls::set_native_control_enabled(view, config.enabled);
    ...
    fn update_table_view(
        ...
        unsafe {
            let rows = config
                .items
                .iter()
                .map(|item| super::native_controls::IosTableRow {
                    text: (*item).to_string(),
                })
                .collect::<Vec<_>>();
    ...
    fn update_collection_view(
        ...
        unsafe {
            let items = config
                .items
                .iter()
                .map(|item| super::native_controls::IosCollectionItem {
                    text: (*item).to_string(),
                })
                .collect::<Vec<_>>();
